function [] = CalculatePlacefields_alt(RoomStr,turn_inds)
...
%% WM Edits - Animal trajectory rotation. 
try 
    load(fullfile(pwd,'rotated.mat')); 
catch
    [~,rot_x,rot_y] = sections(x,y);     
end
...
for i = 1:NumNeurons
  TMap{i} = calcmapdec(FT(i,:),RunOccMap,Xbin,Ybin,(isrunning & turn_inds));
