function [] = MakeNeurons_WM(lengthcrit)
% [] = MakeNeurons()
...
load(['Segments',num2str(lengthcrit),'.mat']); %NumSegments SegChain cc NumFrames Xdim Ydim --- not loading and passing here breaks parallelization
if (exist(['InitClu',num2str(lengthcrit),'.mat'],'file') == 0)
    InitializeClusters_WM(NumSegments, SegChain, cc, NumFrames, Xdim, Ydim, lengthcrit);
end

load(['InitClu',num2str(lengthcrit),'.mat']); %c Xdim Ydim PixelList Xcent Ycent frames meanareas meanX meanY NumEvents cToSeg
NumIterations = 0;
NumCT = length(c);
oldNumCT = NumCT;
...
save(['ProcOut',num2str(lengthcrit),'.mat'],'NeuronImage', 'NeuronPixels', 'NumNeurons', 'c', 'Xdim', 'Ydim', 'FT', 'NumFrames', 'NumTransients', 'MinPixelDist', 'DistUsed', 'InitPixelList', 'VersionString', 'GoodTrs', 'nToc', 'cTon', '-v7.3');
