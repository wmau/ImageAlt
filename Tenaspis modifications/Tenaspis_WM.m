function [] = Tenaspis_WM(infile,varargin)
...
for lengthcrit=2:10
    disp(['Running MakeTransients() with frame span criterion = ',num2str(lengthcrit)]);
    MakeTransients_WM('D1Movie.h5',0,lengthcrit);
    disp(['Running MakeNeurons() with frame span criterion = ',num2str(lengthcrit)]);
    MakeNeurons_WM(lengthcrit);
end
