...
%% Navigate to folder and load data. 
    ...
    elseif plot_type == 8
        load('Tmaps_LR.mat')
        for j = 1:length(Tleft)
            [~, Tleft_nan{j}] = make_nan_TMap(RunOccMap, Tleft{j});
            [~, Tright_nan{j}] = make_nan_TMap(RunOccMap, Tright{j});
        end
    end
...
%%  Get statistically significant splitters
    ...
    if plot_type ~= 4 && plot_type ~= 7 && plot_type ~= 8
        plotSplitters(cellRespsByTrialType,tuningcurves,deltacurve,sigcurve,...
            ...
            invert_raster_color);
    elseif plot_type == 4 || plot_type == 7
        plotSplitters(cellRespsByTrialType,tuningcurves,deltacurve,sigcurve,...
            sigSplitters,savepdf,'plot_type',plot_type,TMap_nan,'invert_raster_color',...
            invert_raster_color);
    elseif plot_type == 8
        plotSplitters(cellRespsByTrialType,tuningcurves,deltacurve,sigcurve,...
            sigSplitters,savepdf,'plot_type',plot_type, Tleft_nan, Tright_nan,'invert_raster_color',...
            invert_raster_color);
    end
end